...
async function addProductToCart(req, res) {
  try {
    ...
    const cartUserBefore = await usersDao.getUserByCart(idCart);
    console.log('usuario asociado al carrito antes', cartUserBefore)
    
    await cartsDao.saveProductToCart(idCart, idProduct);
    const cartUser = await usersDao.getUserByCart(idCart);
    console.log('usuario asociado al carrito', cartUser)
...
async function purchaseProduct(req, res) {
  ...
  try {
    // Obtén el carrito y sus productos asociados
    //const cart = await Cart.findById(cid).populate('products.product');

    const id = req.params.cid;
    let cart = await cartsDao.getCartById(id);
    console.log('Este es el carrito a comprar',cart);
    ...
    for (const item of cart.products) {
      ...
      if (product.stock >= quantity) {
        ...
        // Si el producto no tiene suficiente stock, retorna un mensaje de error
        return res.status(400).json({ error: `No hay suficiente stock para el producto: ${product.title}` });
      }
    }

    // Si todos los productos tienen suficiente stock, el proceso de compra se completa correctamente
    // Aquí puedes realizar cualquier otra lógica necesaria, como generar una factura, etc.

    res.status(200).json({ message: 'Proceso de compra completado con éxito' });
  } catch (error) {
